...
本文档所在分支为 **`stm32h5f4p256-usart6-bl2-public-key-ps256-1MNS-test`**（基于 `1MNS`）。S 在致命 Fault / `tfm_core_panic` 时会往 USART6 打 `PANIC` 行（CFSR/HFSR/SFSR 等），然后 **halt 不复位**，方便抓 TLS Certificate 阶段的静默崩溃。正式量产不要用本开关（`CONFIG_TFM_HALT_ON_CORE_PANIC`）。

### 相对 `stm32h5f4p256-usart6-bl2-public-key-ps256` 改了什么（`1MNS` 布局 + 本 test 调试）
